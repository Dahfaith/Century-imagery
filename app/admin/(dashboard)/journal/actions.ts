...
export async function saveJournalPost(id: string | null, data: any) {
  ...
  try {
    ...
    if (id) {
      const { error } = await (supabase.from('journal_posts') as any)
        .update(updateData)
        .eq('id', id)
      if (error) throw error
    } else {
      const { data: newPost, error } = await (supabase.from('journal_posts') as any)
        .insert(updateData)
        .select()
        .single()
      if (error) throw error
...
export async function seedJournalPosts() {
  await requireAuth(['super_admin', 'admin'])
  const supabase = await createClient()
